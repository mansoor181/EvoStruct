"""Evaluate EvoStruct predictions with the full CHIMERA-Bench metric suite.
...
def load_numbering_scheme():
    """Load numbering scheme from conf/config.yaml."""
    config_path = os.path.join(_SCRIPT_DIR, "conf", "config.yaml")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate EvoStruct predictions with full CHIMERA metric suite",
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--predictions", type=Path,
                       help="Single predictions directory (.pt files)")
    group.add_argument("--aggregate", action="store_true",
                       help="Aggregate all CDR runs from results_dir/evostruct/")
    ...
    if args.predictions:
        ...
        results_root = Path(shared["paths"]["results_root"]) / "evostruct"
